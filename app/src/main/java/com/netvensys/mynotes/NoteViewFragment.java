...
public class NoteViewFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //icon.setImageResource(Note.categoryToDrwawable(noteCat));

        final Resources res = getContext().getResources();
        final int tileSize = res.getDimensionPixelSize(R.dimen.letter_tile_size);

        final LetterTileProvider tileProvider = new LetterTileProvider(getContext());
        final Bitmap letterTile = tileProvider.getLetterTile(noteCat.name(), "key", tileSize, tileSize);

        icon.setImageBitmap((new BitmapDrawable(res, letterTile).getBitmap()));
